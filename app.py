...
# 设置页面配置
st.set_page_config(page_title="全能工程查询助手 v2.7", page_icon="🛠️", layout="wide")
...
with tab1:
    st.header("🔍 键槽公差查询")
    st.caption("输入示例: **8JS9** (公制) 或 **0.25**, **1/4** (英制)")
    ...
    # --- 英制逻辑 ---
        ...
        try:
            ...
            tol_mm_upper = tol_imp_upper * 25.4
            
            # 计算最大/最小极限的绝对公制值
            val_mm_max = (val_imp + tol_imp_upper) * 25.4
            val_mm_min = val_imp * 25.4 # 下偏差为0

            st.divider()
            
            # 1. 显示三栏 (Value中增加 mm 显示)
            ...
            ic2.metric("最大极限 (Max)", 
                       f"{(val_imp + tol_imp_upper):.4f}\" ({val_mm_max:.3f} mm)", 
                       f"+{tol_imp_upper}\"")

            ic3.metric("最小极限 (Min)", 
                       f"{val_imp:.4f}\" ({val_mm_min:.3f} mm)", 
                       f"-0.0000\"")
...
with tab2:
    ...
    if st.button("查询 ISO", type="primary"):
        ...
        if idx != -1 and iso_code in DATA_TABLE_ISO:
            ...
            if is_empty:
                st.error(f"⚠️ {iso_code} 在尺寸 {iso_size}mm 处暂无标准数据。")
            else:
