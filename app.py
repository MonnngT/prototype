...
# 设置页面配置
st.set_page_config(page_title="全能工程查询助手", page_icon="🛠️", layout="wide")

st.title("🛠️ 全能工程查询助手")
st.markdown("集成：**ISO 286 公差** | **公制键槽 (Metric)** | **英制键槽 (Imperial)** | **单位转换**")
...
with tab1:
    ...
    st.caption("数据源：ISO 286-1 标准 (0-3150mm)")
    ...
    with col1:
        iso_size = st.number_input("输入公称尺寸 (mm)", 0.1, 3150.0, 1000.0, 10.0)
    with col2:
        iso_code = st.selectbox("公差带", ["h14", "h12", "h8", "h7", "g8", "H7", "H8", "F7", "G7", "K7"])
...
# ==============================================================================
# TAB 2: 公制键槽 (BS 4235 / ISO)
# ==============================================================================
with tab2:
    st.header("公制键槽 (Metric Keyway)")
    st.caption("输入轴径 (mm) -> 查询键槽标准 (mm)")
...
with tab3:
    ...
    with c_imp1:
        imp_shaft = st.number_input("输入英制轴径 (inch)", 0.2362, 4.3307, 1.000, 0.001, format="%.4f")
    with c_imp2:
        st.write("")
        st.write("")
        btn_imp = st.button("查询英制键槽", type="primary")
    ...
    if btn_imp:
        ...
        if not found:
            st.error("⚠️ 未找到匹配数据，请确认轴径在 0.2362 - 4.3307 inch 之间")

# ==============================================================================
# TAB 4: 键槽单位转换 (Inch Fraction -> Metric)
# ==============================================================================
with tab4:
    st.header("键槽单位转换 (Inch Fraction -> Metric)")
    st.caption("选择英制分数键宽 -> 获取公制对应尺寸及公差")
    ...
    inch_sel = st.selectbox("选择英制键宽", list(INCH_KEY_DB.keys()))
    if st.button("开始转换"):
        data = INCH_KEY_DB[inch_sel]
        st.success(f"转换结果: {inch_sel}")
        m1, m2 = st.columns(2)
        m1.metric("公制宽 (W)", f"{data[0]:.4f} mm", "+0.025")
        m2.metric("公制深 (T2)", f"{data[1]:.3f} mm", "+0.152")
